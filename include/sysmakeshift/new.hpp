...
namespace sysmakeshift
{
    ...
    // As such, they are not particularly useful, and are hence not provided by sysmakeshift. Instead, determine an accurate value at runtime by calling
    // `hardware_cache_line_size()`.
    //
    // Defining these constexpr values in sysmakeshift would be less controversial than defining them in the standard library because sysmakeshift is not
    ...
    // sub-architectures, so a constexpr value defined here should depend only on architecture (e.g. x64), not on sub-architecture (e.g. Intel "Penryn").
